// backend/verify-cashfree-v2-final.js
// 🎯 FINAL CASHFREE PAYOUTS V2 INTEGRATION VERIFICATION
// This script verifies the complete Cashfree Payouts V2 integration with proper authentication

require('dotenv').config();
const axios = require('axios');
const crypto = require('crypto');

class CashfreeV2FinalVerifier {
  constructor() {
  ...
  log(level, message, data = null) {
    ...
    console.log(`${emoji[level]} [${timestamp}] [CASHFREE_V2_FINAL] ${message}`);
    if (data) {
  ...
  generateReport() {
    ...
    require('fs').writeFileSync(
      './cashfree-v2-final-verification-report.json',
      JSON.stringify(reportData, null, 2)
    );

    this.log('info', 'Detailed verification report saved to cashfree-v2-final-verification-report.json');
  }
...
if (require.main === module) {
  const verifier = new CashfreeV2FinalVerifier();
  verifier.runVerification().catch(console.error);
}

module.exports = CashfreeV2FinalVerifier;
